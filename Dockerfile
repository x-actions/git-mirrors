...
RUN apk update && \
    ...
    rm -rf /var/cache/apk/* && \
    cd /tmp/ && \
    wget https://github.com/x-actions/git-mirrors/releases/download/${VERSION}/git-mirrors-linux-amd64 && \
    chmod +x /tmp/git-mirrors-linux-amd64 && \
    mv /tmp/git-mirrors-linux-amd64 /usr/local/bin/git-mirrors
